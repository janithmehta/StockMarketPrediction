...
def update_one(company):
	...
	comp="NSE/"+company
	x=quandl.get([comp+".0",comp+".1",comp+".2",comp+".3",comp+".5",comp+".6"], authtoken="juX5Ui2cwcYoMP8yVR2F",rows=1, returns="pandas")
	...
	ds_rev=ds[::-1]
	ds_rev.to_csv(settings.MEDIA_ROOT+'temp.csv')
	df=pd.read_csv(settings.MEDIA_ROOT+'temp.csv')
